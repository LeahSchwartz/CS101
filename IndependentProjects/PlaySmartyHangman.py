...
def fileToStringList(filename):
    #filename is file, returns list of strings, string represents one line from filename
    wordlist = []
    f = open(filename)
    for line in f:
        line = line.strip()
...
def displayGuess(wordList):
    #wordList is list w/ letters correctly guessed and '_' for letters not quessed yet
    #returns the list as a String
    return ' '.join(wordList)

def guessStart(word):
    #returns a list of single characters '_' the same size as word
    return ['_']*len(word)
...
def changetheWord(wordsOfLength, letter, gameMode, lettersNotGuessed ):
    #makes smart game that changes word after each guess
    newStorage = {}
    biggestCat = 0
    choosenCat = ""
    global DEBUG
    for word in wordsOfLength:
        blankWord = ""
        for ch in word:
            if ch != letter and ch in lettersNotGuessed:
                blankWord += "_" 
            else:
                blankWord += ch
        if blankWord not in newStorage:#builds dictionary with categories of word types
            newStorage[blankWord] = [word]
        else:
            newStorage[blankWord] += [word]
    for key, value in newStorage.items():
        if len(value) > biggestCat:
            biggestCat = len(value)
            choosenCat = key
    wordToGuess = random.choice(newStorage[choosenCat])
    wordsOfLength = newStorage[choosenCat]
    if DEBUG:#prints for test mode
        print "Dictionary of categories and # of words:"
        for key,value in newStorage.items():
            print key, len(value)
        print "(Secret word:",wordToGuess,")   # words possible:", len(wordsOfLength)   
    return wordToGuess, wordsOfLength #returns new word and new list of possible words
...
def playGame(words):
    ...
    gameMode = (raw_input("Type t or g: "))
    while True:#makes sure user types t or g
        if gameMode != "t" and gameMode != "g":
            print "Please type t or g"
            gameMode = (raw_input("Type t or g: "))
        else:
            break
    if gameMode == "t":
    ...
    while True:#makes sure input is a number 3 or more 
        if not str(guessLength).isdigit() or int(guessLength) < 3: 
    ...
    while True:
        ...
        if letter == "+":
            ...
            if wordGuess == wordToGuess and len(wordsOfLength) == 1: #wrong or right if they guessed the word
                print "You win. You guessed the word", wordToGuess 
                break
            else:
                print "You lost. The correct word was", random.choice(wordsOfLength)
                break
        else: #handles already guessed letters and normal letters
            ...
            else: #updates after guess if letter not yet guessed
                newWordandList = changetheWord(wordsOfLength, letter, gameMode, lettersNotGuessed ) #intermediate variable
                wordToGuess = newWordandList[0]#sets new word to guess
                wordsOfLength = newWordandList[1]#sets new list of words
                updateLetter(guessList, wordToGuess, letter) 
                lettersNotGuessed = "".join([w for w in lettersNotGuessed if w != letter.lower()])
                print "Letters not guessed are:", lettersNotGuessed
                if letter not in wordToGuess: #handles wrong guess letter
                    Misses += 1
                print "Number of misses left are:", (missesAllowed - Misses)
...
if __name__ == '__main__':
    ...
    #smallWords = random.sample(words, 50)
    #smallWords = ["apple", "sweat", "crass", "grass", "pearl", "zazzy", "india", "crazy", "minds", "mints", "zebra"]
    playGame(words)
